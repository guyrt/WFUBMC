...
* Y may be referenced by cy (pointer to alglib::complex) or
...
2. for better performance, each row of A should be aligned,
   X should be aligned too. Y may be non-aligned - does not matter.
********************************************************************/
void ialglib::mv_complex(int m, int n, const double *a, const double *x, alglib::complex *cy, double *dy, int stride, alglib::complex alpha, alglib::complex beta)
{
...
It may work with unaligned data.
********************************************************************/
void ialglib::mv_complex_generic(int m, int n, const double *a, const double *x, alglib::complex *cy, double *dy, int stride, alglib::complex alpha, alglib::complex beta)
{
    int i, j;
...
This subroutine sets vector to zero
********************************************************************/
void ialglib::vzero_complex(int n, alglib::complex *p, int stride)
{
    int i;
    if( stride==1 )
    {
...
(passed as alglib::complex*)

1. strideb is stride measured in complex numbers, not doubles
2. conj may be "N" (no conj.) or "C" (conj.)
********************************************************************/
void ialglib::vcopy_complex(int n, const alglib::complex *a, int stridea, double *b, int strideb, char *conj)
{
    int i;
...
* non-contigous
* may be transformed during copying (as prescribed by op)
* pointer to alglib::complex is passed
...
********************************************************************/
void ialglib::mcopyblock_complex(int m, int n, const alglib::complex *a, int op, int stride, double *b)
{
    int i, j;
    const alglib::complex *psrc;
    double *pdst;
    if( op==0 )
    {
...
* 3 - conjugate, but no  transposition
********************************************************************/
void ialglib::mcopyunblock_complex(int m, int n, const double *a, int op, alglib::complex* b, int stride)
{
    int i, j;
    const double *psrc;
    alglib::complex *pdst;
...
bool ialglib::_i_rmatrixgemmf(int m,
     ...
     int k,
     double alpha,
     const alglib::real_2d_array& _a,
     int ia,
     int ja,
     int optypea,
     const alglib::real_2d_array& _b,
     int ib,
     int jb,
     int optypeb,
     double beta,
     alglib::real_2d_array& _c,
     int ic,
     int jc)
{
...
bool ialglib::_i_cmatrixgemmf(int m,
     ...
     const alglib::complex_2d_array& _a,
     int ia,
     int ja,
     int optypea,
     const alglib::complex_2d_array& _b,
     int ib,
     int jb,
     int optypeb,
     alglib::complex beta,
     alglib::complex_2d_array& _c,
     int ic,
     int jc)
 {
    ...
    for(i=0; i<m; i++)
    {
        if( optypea==0 )
        {
            vcopy_complex(k, arow, 1, abuf, 1, (char *) "No conj");
            arow += stride;
        }
        else if( optypea==1 )
        {
            vcopy_complex(k, arow, stride, abuf, 1, (char *) "No conj");
            arow++;
        }
        else
        {
            vcopy_complex(k, arow, stride, abuf, 1, (char *) "Conj");
            arow++;
        }
        if( beta==0 )
     ...
     int j2)
{
    ...
    if( uppera )
    {
        for(i=0,pdiag=abuf; i<n; i++,pdiag+=2*(alglib_c_block+1))
        {
            alglib::complex beta = 1.0/alglib::complex(pdiag[0],pdiag[1]);
            alglib::complex alpha;
            alpha.x = -beta.x;
            alpha.y = -beta.y;
            vcopy_complex(i, abuf+2*i, alglib_c_block, tmpbuf, 1, (char *)"No conj");  // RTG
    ...
    else
    {
        for(i=n-1,pdiag=abuf+2*((n-1)*alglib_c_block+(n-1)); i>=0; i--,pdiag-=2*(alglib_c_block+1))
        {
            ...
            alpha.x = -beta.x;
            alpha.y = -beta.y;
            vcopy_complex(n-1-i, pdiag+2*alglib_c_block, alglib_c_block, tmpbuf, 1, (char *)"No conj"); // RTG
     ...
     int j2)
{
    ...
    if( uppera )
    {
        for(i=m-1,pdiag=abuf+2*((m-1)*alglib_c_block+(m-1)); i>=0; i--,pdiag-=2*(alglib_c_block+1))
        {
            ...
            alpha.x = -beta.x;
            alpha.y = -beta.y;
            vcopy_complex(m-1-i, pdiag+2, 1, tmpbuf, 1, (char *)"No conj"); // RTG
    ...
    {   for(i=0,pdiag=abuf,arow=abuf; i<m; i++,pdiag+=2*(alglib_c_block+1),arow+=2*alglib_c_block)
        {
            ...
            alpha.x = -beta.x;
            alpha.y = -beta.y;
            vcopy_complex(i, arow, 1, tmpbuf, 1,(char *) "No conj"); // RTG
     ...
     bool isupper)
{
    ...
    if( isupper )
    {
        ...
        {
            vcopy_complex(k, arow, 1, tmpbuf, 1, (char *)"Conj"); // RTG changed to remove warnings.
...
bool ialglib::_i_cmatrixrank1f(int m,
     ...
     int voffs)
{
    alglib::complex *arow, *pu, *pv, *vtmp, *dst;
    int n2 = n/2;
...
/*void alglib::mv_32_sse2(const double *a, const double *x, double *y, double alpha, double beta)
{
    int j;
    const double *pa, *pb;
...
/*void alglib::mv_complex_sse2(int m, int n, const double *a, const double *x, alglib::complex *y, alglib::complex alpha, alglib::complex beta)
{
    int i, j, n2;
    const double *pa, *parow, *pb;
    double v0, v1, v2, v3;
